...
const DitheringTool: React.FC<DitheringToolProps> = ({ initialMode = "image" }) => {
  ...
  // Keep active media ids valid even if state was partially reset/replayed.
  useEffect(() => {
    if (!videoMode && images.length > 0) {
      const hasValidActive = !!activeImageId && images.some((img) => img.id === activeImageId);
      if (!hasValidActive) {
        setActiveImageId(images[0].id);
      }
    }
  }, [videoMode, images, activeImageId, setActiveImageId]);

  useEffect(() => {
    if (videoMode && videos.length > 0) {
      const hasValidActive = !!activeVideoId && videos.some((vid) => vid.id === activeVideoId);
      if (!hasValidActive) {
        setActiveVideoId(videos[0].id);
      }
    }
  }, [videoMode, videos, activeVideoId, setActiveVideoId]);
  ...
  const addVideos = (items: { url: string; name?: string; file?: File }[]) => {
    setVideos((prev: UploadedVideo[]) => {
      ...
      const hasValidActive = !!activeVideoId && next.some((video) => video.id === activeVideoId);
      if (!hasValidActive && next.length > 0) {
        setActiveVideoId(next[next.length - items.length]?.id ?? next[0].id);
      }
      return next;
    });
  };
