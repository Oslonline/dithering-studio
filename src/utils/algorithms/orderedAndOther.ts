...
export function runBinaryThreshold(ctx: AlgorithmRunContext) {
  const { srcData, params }=ctx; const out=new Uint8ClampedArray(srcData); for(let i=0;i<out.length;i+=4){ const v= out[i] < params.threshold ? 0:255; out[i]=out[i+1]=out[i+2]=v; out[i+3]=255; } if(params.invert){ for(let i=0;i<out.length;i+=4){ out[i]=255-out[i]; out[i+1]=255-out[i+1]; out[i+2]=255-out[i+2]; } } return out; }

export function runRandomThreshold(ctx: AlgorithmRunContext) {
  const { srcData,width,height,params }=ctx; const out=new Uint8ClampedArray(srcData); const userT=params.threshold ?? 128; // Interpret slider as shifting probability pivot
  // Effective probability threshold: higher slider -> more white pixels
  for(let y=0;y<height;y++) for(let x=0;x<width;x++){ const i=(y*width+x)*4; const g=out[i]; const pivot = userT/255; const r=Math.random(); // compare normalized gray + noise vs pivot
    const normalized=g/255; // blend gray and random noise to keep tonal relation but allow threshold shift
    const mix = (normalized + r)/2; const v = mix < pivot ? 0:255; out[i]=out[i+1]=out[i+2]=v; out[i+3]=255; }
...
export function runDotDiffusionSimple(ctx: AlgorithmRunContext) {
  const { srcData,width,height,params }=ctx; const out=new Uint8ClampedArray(srcData); for(let y=0;y<height;y++) for(let x=0;x<width;x++){ const i=(y*width+x)*4; const mask=((x&1)^(y&1))? params.threshold : 255-params.threshold; const v= out[i] < mask ? 0:255; out[i]=out[i+1]=out[i+2]=v; out[i+3]=255; } if(params.invert){ for(let i=0;i<out.length;i+=4){ out[i]=255-out[i]; out[i+1]=255-out[i+1]; out[i+2]=255-out[i+2]; } } return out; }

export function runHalftone(ctx: AlgorithmRunContext) {
  const { srcData,width,height,params }=ctx; const out=new Uint8ClampedArray(srcData); const dotSize=6; for(let y=0;y<height;y+=dotSize){ for(let x=0;x<width;x+=dotSize){ let sum=0; let count=0; for(let dy=0;dy<dotSize;dy++) for(let dx=0;dx<dotSize;dx++){ const nx=x+dx; const ny=y+dy; if(nx<width && ny<height){ const i=(ny*width+nx)*4; sum+=out[i]; count++; } } const avg=sum/count; const isDot=avg < params.threshold; for(let dy=0;dy<dotSize;dy++) for(let dx=0;dx<dotSize;dx++){ const nx=x+dx; const ny=y+dy; if(nx<width && ny<height){ const i=(ny*width+nx)*4; const cx=dotSize/2-0.5; const cy=dotSize/2-0.5; const dist=Math.sqrt((dx-cx)**2+(dy-cy)**2); const radius=isDot? dotSize/2.2 : dotSize/3.5; const v= dist < radius ? 0:255; out[i]=out[i+1]=out[i+2]=v; out[i+3]=255; } } } } if(params.invert){ for(let i=0;i<out.length;i+=4){ out[i]=255-out[i]; out[i+1]=255-out[i+1]; out[i+2]=255-out[i+2]; } } return out; }

export function runFalseFloydSteinberg(ctx: AlgorithmRunContext) {
  const { srcData,width,height,params }=ctx; const out=new Uint8ClampedArray(srcData); const serp=!!params.serpentine; const distribute=(x:number,y:number,err:number,l2r:boolean)=>{ if(l2r){ if(x+1<width){ const i=(y*width+(x+1))*4; out[i]+= (err*3)/8; out[i+1]+= (err*3)/8; out[i+2]+=(err*3)/8; } if(y+1<height){ { const i=((y+1)*width+x)*4; out[i]+= (err*3)/8; out[i+1]+= (err*3)/8; out[i+2]+=(err*3)/8; } if(x+1<width){ const i=((y+1)*width+(x+1))*4; out[i]+= (err*2)/8; out[i+1]+= (err*2)/8; out[i+2]+=(err*2)/8; } } } else { if(x-1>=0){ const i=(y*width+(x-1))*4; out[i]+= (err*3)/8; out[i+1]+= (err*3)/8; out[i+2]+=(err*3)/8; } if(y+1<height){ { const i=((y+1)*width+x)*4; out[i]+= (err*3)/8; out[i+1]+= (err*3)/8; out[i+2]+=(err*3)/8; } if(x-1>=0){ const i=((y+1)*width+(x-1))*4; out[i]+= (err*2)/8; out[i+1]+= (err*2)/8; out[i+2]+=(err*2)/8; } } } };
  for(let y=0;y<height;y++){ const l2r = !serp || y%2===0; if(l2r){ for(let x=0;x<width;x++){ const idx=(y*width+x)*4; const oldV=out[idx]; const newV= oldV < params.threshold ? 0:255; const err=oldV-newV; out[idx]=out[idx+1]=out[idx+2]=newV; distribute(x,y,err,true);} } else { for(let x=width-1;x>=0;x--){ const idx=(y*width+x)*4; const oldV=out[idx]; const newV= oldV < params.threshold ? 0:255; const err=oldV-newV; out[idx]=out[idx+1]=out[idx+2]=newV; distribute(x,y,err,false);} } }
  if (params.invert){ for(let i=0;i<out.length;i+=4){ out[i]=255-out[i]; out[i+1]=255-out[i+1]; out[i+2]=255-out[i+2]; } }
  return out;
}

export function runAdaptiveOstromoukhov(ctx: AlgorithmRunContext) {
  const { srcData,width,height,params }=ctx; const out=new Uint8ClampedArray(srcData); const kernels:{w:number[]}[]=[ {w:[7,3,5,1]},{w:[5,3,3,1]},{w:[4,3,2,1]},{w:[3,2,2,1]},{w:[2,2,1,1]} ];
  for(let y=0;y<height;y++){ const serp=params.serpentine && (y%2===1); for(let xi=0; xi<width; xi++){ const x=serp? (width-1-xi):xi; const idx=(y*width+x)*4; const old=out[idx]; const neu= old < params.threshold ? 0:255; const err=old-neu; out[idx]=out[idx+1]=out[idx+2]=neu; const bucket=Math.min(4,Math.max(0,Math.floor(old/51))); const weights=kernels[bucket].w; const denom=weights.reduce((a,b)=>a+b,0); const coords=serp ? [[x-1,y],[x-2,y],[x,y+1],[x-1,y+1]] : [[x+1,y],[x+2,y],[x,y+1],[x+1,y+1]]; for(let c=0;c<coords.length;c++){ const [nx,ny]=coords[c]; if(nx>=0&&nx<width&&ny>=0&&ny<height){ const nIdx=(ny*width+nx)*4; const dif=(err*weights[c])/denom; out[nIdx]+=dif; out[nIdx+1]+=dif; out[nIdx+2]+=dif; } } } }
  if(params.invert){ for(let i=0;i<out.length;i+=4){ out[i]=255-out[i]; out[i+1]=255-out[i+1]; out[i+2]=255-out[i+2]; } }
  return out;
}
