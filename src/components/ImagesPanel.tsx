  ...
  return (
    <div className="min-panel p-0">
      <button type="button" onClick={() => setOpen(o => !o)} className="flex w-full items-center justify-between px-4 py-3 text-left font-mono text-[11px] tracking-wide text-gray-300 hover:bg-neutral-800/40 focus-visible:shadow-[var(--focus-ring)]">
        <span className="flex items-center gap-2"><span>{open ? '▾' : '▸'}</span> {t('tool.imagesPanel.title')}</span>
        ...
        <div className="space-y-3 border-t border-neutral-800 px-4 pt-3 pb-4">
          <div className="flex gap-2">
            <label className="clean-btn flex-1 justify-center text-[10px] cursor-pointer">
              ...
              {t('tool.imagesPanel.add')}
            </label>
            {images.length > 0 && (
              <button type="button" onClick={clearAll} className="clean-btn flex-1 justify-center text-[10px]" title={t('tool.imagesPanel.clearAllHint')}>{t('tool.imagesPanel.clear')}</button>
            )}
          </div>
          {images.length === 0 && <p className="text-[10px] text-gray-500">{t('tool.imagesPanel.noImages')}</p>}
          {images.length > 0 && (
                ...
                return (
                  ...
                  >
                    <button type="button" onClick={() => setActiveId(img.id)} className="flex items-center gap-2 text-left focus-visible:shadow-[var(--focus-ring)]" tabIndex={0}>
                      ...
                      <span className="max-w-[120px] truncate font-mono text-[10px] text-gray-300">{img.name || t('tool.imagesPanel.image')}</span>
                    </button>
                    <button type="button" onClick={() => removeImage(img.id)} className="ml-auto hidden cursor-pointer text-[10px] text-gray-500 hover:text-red-400 group-hover:inline" title={t('tool.imagesPanel.remove')}>✕</button>
                  </li>
                );
              })}
            </ul>
