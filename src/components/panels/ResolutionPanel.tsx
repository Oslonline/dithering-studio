  ...
  return (
    <div className="min-panel p-0">
      <button type="button" onClick={() => setOpen(o => !o)} className="flex w-full items-center justify-between px-4 py-3 text-left font-mono text-[11px] tracking-wide text-gray-300 hover:bg-neutral-800/40 focus-visible:shadow-[var(--focus-ring)]" aria-expanded={open}>
        <span className="flex items-center gap-2"><span>{open ? '▾' : '▸'}</span> {t('tool.tonePanel.workingResolution')}</span>
        <span className="text-[10px] text-gray-500">{workingResolution}{t('tool.tonePanel.px')}</span>
      </button>
      {open && (
        <div className="space-y-2 border-t border-neutral-800 px-4 pt-3 pb-4">
          ...
          <div className="flex items-center gap-2">
            ...
            }} onBlur={() => { if (workingResInput === '') setWorkingResInput(String(workingResolution)); }} aria-label={t('tool.tonePanel.workingResolution')} />
            <span className="text-[10px] text-gray-500">{t('tool.tonePanel.px')} width</span>
          </div>
          <p className="text-[10px] text-gray-500">{t('tool.tonePanel.widthHint', { max: hardMax })}</p>
        </div>
      )}
    </div>
  );
