...
const ResolutionPanel: React.FC<ResolutionPanelProps> = ({ workingResolution, setWorkingResolution, workingResInput, setWorkingResInput }) => {
      ...
      {open && (
        <div className="space-y-2 border-t border-neutral-800 px-4 pt-3 pb-4">
          <div className="relative">
            <input type="range" min={32} max={2048} step={16} value={workingResolution} className="clean-range" onChange={(e) => { const val = Number(e.target.value); setWorkingResolution(val); setWorkingResInput(String(val)); }} />
          </div>
          <div className="flex items-center gap-2">
            <input type="text" inputMode="numeric" pattern="[0-9]*" className="clean-input w-24" value={workingResInput} onChange={(e) => {
              ...
              if (/^\d+$/.test(v)) { let num = parseInt(v, 10); num = Math.min(4096, Math.max(16, num)); setWorkingResInput(String(num)); setWorkingResolution(num); }
