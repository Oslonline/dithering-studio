...
interface Props { focusMode: boolean; }

const FocusHint: React.FC<Props> = ({ focusMode }) => {
  ...
  useEffect(() => {
    const calc = () => {
      if (focusMode) { setStyle({ left: 8, top: 8 }); return; }
  ...
  return (
    <div className="pointer-events-none fixed z-20 rounded bg-neutral-900/70 px-3 py-1 font-mono text-[10px] tracking-wide text-gray-300 shadow transition-all duration-150" style={{ left: style.left, top: style.top }}>
